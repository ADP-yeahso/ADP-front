...
class DiaryLoadingScreen extends StatefulWidget {
  const DiaryLoadingScreen({super.key, this.loadNext});
...
class _DiaryLoadingScreenState extends State<DiaryLoadingScreen> {
  ...
  void initState() {
    ...
    if (widget.loadNext == null) {
      _showResultAfterDelay();
    } else {
      _load();
    }
  }

  Future<void> _showResultAfterDelay() async {
    await Future<void>.delayed(const Duration(seconds: 3));
    if (!mounted) return;
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (_) => const DiaryResultScreen()),
    );
  }
