  ...
  Future<void> _confirmLogout(BuildContext context) async {
    ...
    if (confirmed == true && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('로그아웃 되었어요 (데모 모드)')),
      );
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Container(
            padding: const EdgeInsets.all(20),
            decoration: BoxDecoration(color: const Color(0xFFF7F5EE), borderRadius: BorderRadius.circular(20)),
            child: Row(
              children: [
                CircleAvatar(
                  radius: 28,
          ...
          const SizedBox(height: 20),
          const Text('설정', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Colors.black45)),
          ...
          _SettingsTile(
            ...
            label: '기록 공개 범위 관리',
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const VisibilitySettingsScreen())),
          ),
          _SettingsTile(
            icon: Icons.family_restroom_outlined,
            label: '가족 구성원 관리',
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const FamilyManagementScreen())),
          ),
          _SettingsTile(
...
class _StatRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        Expanded(child: _StatCard(label: '작성한 일기', value: '$myDiaries개', icon: Icons.local_florist_outlined)),
        const SizedBox(width: 12),
        Expanded(child: _StatCard(label: '나무 기록', value: '$myLeaves개', icon: Icons.park_outlined)),
      ],
    );
  }
}
